...
4. **Record Rounds**: Tap the Rounds tab to score rounds hole-by-hole, solo or with other players. For same-device multiplayer, add players and pass the device. For multi-device rounds, toggle on multi-device mode, share the QR invite, and score independently on each device. Track your scores over time. Optionally post your scorecards to Nostr.
...
- **Multi-device rounds require Nostr**: Multi-device score sharing uses Nostr relays for coordination. Both devices must have working relay connections. If you experience issues joining or syncing scores, check your relay settings in the profile view.
